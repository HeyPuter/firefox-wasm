...
/** @type {function(...*):?} */ function _MozWalkTheStack() {
...
var NODERAWFS = {
 ...
 read(stream, buffer, offset, length, position) {
  ...
  var bytesRead = fs.readSync(stream.nfd, new Int8Array(buffer.buffer, offset, length), {
   position: position
  });
 ...
 write(stream, buffer, offset, length, position) {
  ...
  var seeking = typeof position != "undefined";
  if (!seeking && stream.seekable) position = stream.position;
  var bytesWritten = fs.writeSync(stream.nfd, new Int8Array(buffer.buffer, offset, length), {
   position: position
  });
...
if (ENVIRONMENT_IS_NODE) {
 ...
 /** @suppress {partialAlias} */ var VFS = Object.assign({}, FS);
