...
async function createEmbed(moduleArg = {}) {
...
function _MozWalkTheStack(...args) {
  abort("missing function: MozWalkTheStack");
}

_MozWalkTheStack.stub = true;

function __ZNK2js12NativeObject15numDynamicSlotsEv(...args) {
...
function whSyncMem(h, dir) {
  ...
  if (dir === 0) hb.set(HEAPU8.subarray(mir.ptr >>> 0, mir.ptr + n >>> 0)); else HEAPU8.set(hb.subarray(0, n), mir.ptr >>> 0);
}
...
function _wasmhost_call(h, idx, argsptr, argc) {
  ...
  whSyncMem(h, 0);
  // guest -> host before the export runs
...
function _wasmhost_export_register_obj(...args) {
  abort("missing function: wasmhost_export_register_obj");
}

_wasmhost_export_register_obj.stub = true;

function _wasmhost_global_get(...args) {
  abort("missing function: wasmhost_global_get");
}

_wasmhost_global_get.stub = true;
...
function _wasmhost_global_set(...args) {
  abort("missing function: wasmhost_global_set");
}

_wasmhost_global_set.stub = true;
...
function _wasmhost_instantiate(h, callbackIdsPtr, importCount) {
  try {
    ...
    function shimFor(cbid, hh) {
      return function() {
        whSyncMem(hh, 1);
        // host -> guest, so the guest import sees live memory
        ...
        for (var i = 0; i < argc; i++) {
          var a = arguments[i];
          HEAPF64[base + i >>> 0] = typeof a === "bigint" ? Number(a) : a;
        }
        var ret = Module._wasmhost_invoke_import(cbid, scratch, argc);
        whSyncMem(hh, 0);
        // guest -> host, capture the import's writes
        return ret;
      };
    }
    var hostImports = {};
    for (var i = 0; i < importCount; i++) {
      ...
      var id = HEAP32[(callbackIdsPtr >> 2) + i >>> 0];
      if (id === -2) {
...
function _wasmhost_table_call(...args) {
  abort("missing function: wasmhost_table_call");
}

_wasmhost_table_call.stub = true;

function _wasmhost_table_grow(...args) {
  abort("missing function: wasmhost_table_grow");
}

_wasmhost_table_grow.stub = true;

function _wasmhost_table_length(...args) {
  abort("missing function: wasmhost_table_length");
}

_wasmhost_table_length.stub = true;
...
function _wasmhost_table_set_hostfn(...args) {
  abort("missing function: wasmhost_table_set_hostfn");
}

_wasmhost_table_set_hostfn.stub = true;

function _wasmhost_table_set_null(...args) {
  abort("missing function: wasmhost_table_set_null");
}

_wasmhost_table_set_null.stub = true;
...
var _main, _free, _malloc, _WJTraceRoots, _wjhelp, _wasmjit_invoke, _InterpTraceRoots, _wasmhost_invoke_import, _emscripten_builtin_memalign, __emscripten_stack_restore, __emscripten_stack_alloc, _emscripten_stack_get_current, memory, __indirect_function_table, wasmMemory, wasmTable;

function assignWasmExports(wasmExports) {
  _main = Module["_main"] = wasmExports["__main_argc_argv"];
  _free = Module["_free"] = wasmExports["free"];
  _malloc = Module["_malloc"] = wasmExports["malloc"];
  _WJTraceRoots = Module["_WJTraceRoots"] = wasmExports["WJTraceRoots"];
  _wjhelp = Module["_wjhelp"] = wasmExports["wjhelp"];
  _wasmjit_invoke = Module["_wasmjit_invoke"] = wasmExports["wasmjit_invoke"];
  _InterpTraceRoots = Module["_InterpTraceRoots"] = wasmExports["InterpTraceRoots"];
  _wasmhost_invoke_import = Module["_wasmhost_invoke_import"] = wasmExports["wasmhost_invoke_import"];
...
var wasmImports = {
  /** @export */ MozWalkTheStack: _MozWalkTheStack,
  ...
  /** @export */ wasmhost_export_register_obj: _wasmhost_export_register_obj,
  /** @export */ wasmhost_global_get: _wasmhost_global_get,
  /** @export */ wasmhost_global_new: _wasmhost_global_new,
  /** @export */ wasmhost_global_set: _wasmhost_global_set,
  ...
  /** @export */ wasmhost_table_call: _wasmhost_table_call,
  /** @export */ wasmhost_table_grow: _wasmhost_table_grow,
  /** @export */ wasmhost_table_length: _wasmhost_table_length,
  /** @export */ wasmhost_table_new: _wasmhost_table_new,
  /** @export */ wasmhost_table_set_hostfn: _wasmhost_table_set_hostfn,
  /** @export */ wasmhost_table_set_null: _wasmhost_table_set_null
};
...
if (typeof exports === 'object' && typeof module === 'object') {
  ...
  module.exports.default = createEmbed;
