// Local page runner: drives system chromium via playwright-core.
// Env: PAGE (bench html), MARK (title substring to wait for), QS (extra index.html
// query, e.g. "env.GECKO_WJVS_FRAME=1"), NOJIT=1 to disable the JIT.
const {chromium}=require("playwright-core");
const {startBenchServer}=require("./bench-server.cjs");
const sleep=(ms)=>new Promise(r=>setTimeout(r,ms));
const PAGE = process.env.PAGE || "jisetp.html";
const MARK = process.env.MARK || "SETP ";
...
(async()=>{
 ...
 const grab=(src)=>(m)=>{const t=(m&&m.text&&m.text())||String(m); if(/compile failed|RuntimeError|out of bounds|unreachable|trap|abort|memory access|\[wasm/i.test(t))console.log(src+" "+t.slice(0,180));};
 p.on('console',grab('CON')); p.on('pageerror',e=>console.log("PAGEERR "+String(e).slice(0,180)));
 p.on('crash',()=>console.log("*** PAGE CRASH ***"));
 p.on('worker',w=>w.on('console',grab('WRK')));
 let qs="?";
 if(!process.env.NOJIT) qs+="env.GECKO_DEBUG_JIT=1&jit=1";
 else qs+="env.GECKO_NOWASMJIT=1";
 if(QS) qs+="&"+QS;
