...
(async()=>{
 ...
 const show=(pfx)=>(m)=>{ try{ const t=(m&&m.text&&m.text())||String(m);
   if(/wjtjf|out of bounds|RuntimeError|wasm-function\[/i.test(t)) console.log(pfx+"| "+t); }catch(e){} };
 p.on('console',show("CON")); p.on('worker',w=>w.on('console',show("WRK")));
 await p.goto("http://127.0.0.1:"+port+"/index.html?env.GECKO_WJVS_HASCALL=1&env.WJDBG_TJF=1",{waitUntil:"load",timeout:120000});
 ...
 for(let i=0;i<20;i++){ await sleep(1000); try{ const t=await Promise.race([p.evaluate(()=>window.geckoEval("document.title")),new Promise((_,r)=>setTimeout(()=>r(new Error("to")),2000))]); if((t||"").indexOf("OCTSCORE=")>=0){console.log("DONE "+t);break;} }catch(e){} }
