...
async function run(port, off){
 ...
 await p.evaluate((u)=>window.geckoRender(u),"http://127.0.0.1:"+port+"/bench/jiproto.html");
 let t="";
 for(let i=0;i<150;i++){ await sleep(500); try{t=(await p.evaluate(()=>window.geckoEval("document.title")))||"";}catch(e){} if(t.indexOf("protoms=")>=0)break; }
 await b.close(); return t+(errs.length?(" |ERR "+errs[0]):"");
}
(async()=>{
 const {server,port}=await startBenchServer(0);
 console.log("PROTO JIT ON :", await run(port,false));
 console.log("PROTO JIT OFF:", await run(port,true));
